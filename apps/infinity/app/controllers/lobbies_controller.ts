...
/**
 * Main Lobbies Controller
 *
 * Handles all lobby CRUD operations and business logic:
 * - Create, read, update, delete lobbies
 * - Join/leave operations
 * - Start game
 * - Player management (kick, transfer ownership)
 */
@inject()
export default class LobbiesController {
  private logger = createContextLogger('LobbiesController')
  ...
  async index({ inertia, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to load lobbies')
      return inertia.render('lobbies', {
        lobbies: [],
        user: {
          uuid: user.userUuid,
  ...
  async show({ params, inertia, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to load lobby')
  ...
  async showJoinByInvite({ params, inertia, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to load lobby for invitation')
      return inertia.render('errors/server_error', {
        error: { message: 'Failed to load lobby' },
      })
    }
  ...
  async joinByInvite({ params, request, response, auth, session }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to join lobby by invitation')
  ...
  async join({ params, request, response, auth, session }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to join lobby')
      if (request.accepts(['html'])) {
        session.flash('error', 'Failed to join lobby. Please try again.')
        return response.redirect().back()
      }
  ...
  async leave({ params, request, response, auth, session }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to leave lobby')
      if (request.accepts(['html'])) {
  ...
  async start({ params, request, response, auth, session }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to start game')
      if (request.accepts(['html'])) {
  ...
  async kickPlayer({ params, request, response, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to kick player')
      return response.status(500).json({
  ...
  async transferOwnership({ params, request, response, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to transfer ownership')
      return response.status(500).json({
  ...
  async apiShow({ params, response }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to get lobby')
      return response.status(500).json({
  ...
  async apiIndex({ response }: HttpContext) {
    try {
      ...
      this.logger.error({ error }, 'Failed to get lobbies')
      return response.status(500).json({
  ...
  async destroy({ params, response, auth }: HttpContext) {
    ...
    try {
      ...
      this.logger.error({ error }, 'Failed to delete lobby')
      return response.status(500).json({
  ...
  async leaveOnClose({ request, response, auth }: HttpContext) {
    try {
      ...
      this.logger.error({ error }, 'Failed to leave lobby on close')
      return response.status(500).json({
