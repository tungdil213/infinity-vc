...
router
  .group(() => {
    router
      .get('/login', '#controllers/enhanced_auth_controller.showLogin')
      .as('auth.login.show')
    router
...
router
  .group(() => {
    // Auth actions
    router
      .post('/auth/logout', '#controllers/enhanced_auth_controller.logout')
      .as('auth.logout')
    ...
    router
      ...
      .as('lobbies.create')
    router
      .post('/lobbies', '#controllers/enhanced_lobbies_controller.store')
      .as('lobbies.store')
    router
      .get('/lobbies/:uuid', '#controllers/enhanced_lobbies_controller.show')
      .as('lobbies.show')
    router
    ...
    // Games routes
    router
      .get('/games/:uuid', '#controllers/games_controller.show')
      .as('games.show')
    router
      .post('/games/:uuid/leave', '#controllers/games_controller.leave')
      .as('games.leave')
  })
  .use(middleware.auth())
...
router
  .group(() => {
    // Auth API
    router
      .get('/auth/me', '#controllers/enhanced_auth_controller.me')
      .as('api.auth.me')
    router
      .get('/auth/check', '#controllers/enhanced_auth_controller.check')
      .as('api.auth.check')
    ...
    // Games API
    router
      .get('/games/:uuid', '#controllers/games_controller.apiShow')
      .as('api.games.show')
    router
...
router
  .group(() => {
    router
      .get('/connect', '#controllers/sse_controller.connect')
      .as('sse.connect')
    router
      .post('/subscribe', '#controllers/sse_controller.subscribe')
      .as('sse.subscribe')
    router
      .post('/unsubscribe', '#controllers/sse_controller.unsubscribe')
      .as('sse.unsubscribe')
    router
      .get('/stats', '#controllers/sse_controller.stats')
      .as('sse.stats')
  })
  .prefix('/sse')
  .use(middleware.auth())
