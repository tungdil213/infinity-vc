	...
	render: () => (
		<Card className="w-[300px]">
			...
			<div className="p-4">
				...
				<p className="text-sm text-muted-foreground">A card with an image that maintains its aspect ratio.</p>
			</div>
		</Card>
	),
};
		...
		<div className="grid grid-cols-3 gap-4 w-[900px]">
			{[
				...
				<div key={item.label}>
					...
					<p className="mt-2 text-sm text-center text-muted-foreground">{item.label}</p>
				</div>
			))}
		</div>
	),
		...
		<div className="w-[300px]">
			<AspectRatio ratio={16 / 9}>
				<div className="w-full h-full rounded-md bg-gradient-to-r from-blue-500 to-purple-500 p-8 flex items-center justify-center">
					...
					<h3 className="relative text-2xl font-bold text-white">With Background Pattern</h3>
				</div>
			</AspectRatio>
		</div>
	),
