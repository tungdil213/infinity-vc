			...
			<div className="w-[300px] space-y-4">
				<div className="flex items-center justify-between">
					<div className="space-y-1">
						...
						<p className="text-sm text-muted-foreground">{isComplete ? 'Complete' : `${Math.round(progress)}%`}</p>
					</div>
					{isComplete ? (
				...
				<Button onClick={startDownload} disabled={!isComplete && progress > 0} className="w-full">
					{progress > 0 && !isComplete ? 'Downloading...' : isComplete ? 'Download Again' : 'Start Download'}
				</Button>
			</div>
		);
	},
			...
			<div className="w-[300px] space-y-4">
				...
				<div className="flex justify-between">
					{steps.map((step, index) => (
						<div key={step} className={`text-sm ${index <= currentStep ? 'text-primary' : 'text-muted-foreground'}`}>
							{step}
						</div>
					))}
				</div>
