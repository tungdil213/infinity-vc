			...
			<Card className="p-4">
				<div className="space-y-4">
					<div>
						...
						<p className="text-sm text-muted-foreground">Some dates are disabled for selection</p>
					</div>
					<Calendar
						mode="single"
						selected={date}
...
export const MultipleMonths: Story = {
	render: () => {
		const [date, setDate] = React.useState<Date | undefined>(new Date());
		return (
			<Calendar mode="single" selected={date} onSelect={setDate} numberOfMonths={3} className="rounded-md border" />
		);
	},
};
