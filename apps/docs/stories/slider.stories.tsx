...
import { Slider } from '@infinity.dev/ui/primitives/slider';
	...
	render: function BrightnessSlider() {
		...
		return (
			<div className="w-[200px] space-y-4">
				...
				<Slider defaultValue={[brightness]} max={100} step={1} onValueChange={([value]) => setBrightness(value)} />
			</div>
		);
	},
};
...
export const Vertical: Story = {
	render: () => (
		<div className="flex h-[180px] items-center justify-center">
			<Slider defaultValue={[50]} orientation="vertical" className="h-[150px]" />
		</div>
	),
};
	...
	render: () => (
		<div className="flex h-[180px] items-end space-x-8 rounded-lg border p-8">
			{[
				...
				<Slider key={item.label} defaultValue={[item.value]} orientation="vertical" className="h-[150px]" />
			))}
		</div>
	),
};
