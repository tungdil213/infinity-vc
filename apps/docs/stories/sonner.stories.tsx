...
export const Default: Story = {
	render: () => (
		<div className="flex flex-wrap gap-4">
			...
			<Button onClick={() => toast.success('Successfully created!')}>Success Toast</Button>
			<Button onClick={() => toast.error('Something went wrong!')}>Error Toast</Button>
			<Button onClick={() => toast.info('Deploy has started')}>Info Toast</Button>
					...
					toast.custom((t) => (
						<div className="flex items-center gap-4 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 p-4 text-white">
							...
							<div className="flex-1">
								...
								<p className="text-sm opacity-90">This is a custom styled toast message</p>
							</div>
							<Button variant="secondary" size="sm" onClick={() => toast.dismiss(t)}>
								Dismiss
							</Button>
						</div>
					))
					...
					toast(
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">A new software version is available for installation</p>
							</div>
						</div>,
						{
							action: {
					...
					toast(
						<div className="grid gap-2">
							...
							<p className="text-sm text-muted-foreground">
								Our servers will undergo maintenance on Sunday at 3 AM GMT. Expected duration: 2 hours.
							</p>
