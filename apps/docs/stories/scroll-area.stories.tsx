				...
				<div key={`message-${i}`} className="mb-4">
					<div className="flex items-center gap-3 mb-2">
						<Avatar>
							...
							<AvatarFallback>{['JD', 'AB', 'CK'][i % 3]}</AvatarFallback>
						</Avatar>
						<div>
							<div className="text-sm font-medium">{['John Doe', 'Alice Brown', 'Chris King'][i % 3]}</div>
	...
	render: () => (
		<ScrollArea className="w-96 whitespace-nowrap rounded-md border">
			<div className="flex w-max space-x-4 p-4">
				{Array.from({ length: 50 }).map((_, i) => (
					<div key={`item-${i}`} className="w-[150px] shrink-0 rounded-md border p-4">
					...
					<Card key={`card-${i}`} className="p-4">
						<div className="flex items-center gap-4">
							...
							<div className="grid gap-1">
								<div className="font-medium">{['John Doe', 'Alice Brown', 'Chris King'][i % 3]}</div>
		...
		<ScrollArea className="h-[500px] w-[600px] rounded-md border p-6">
			{Array.from({ length: 10 }).map((_, i) => (
				<div key={`section-${i}`} className="mb-8">
					<h3 className="text-lg font-semibold mb-2">Section {i + 1}: Getting Started</h3>
					...
					<div className="grid gap-2">
						{Array.from({ length: 3 }).map((_, j) => (
							<div key={`example-${i}-${j}`} className="text-sm rounded-md bg-muted p-4">
								Example {j + 1}: Lorem ipsum dolor sit amet, consectetur adipiscing elit.
							</div>
						))}
					</div>
