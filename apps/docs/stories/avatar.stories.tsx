		...
		<div className="flex gap-4">
			{[
				...
				<div key={item.status} className="relative">
					...
					<span className={cn('absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white', item.color)} />
				</div>
			))}
		</div>
	),
		...
		<div className="flex items-center space-x-[-0.75rem]">
			{[1, 2, 3, 4, 5].map((i) => (
				<Avatar key={i} className="border-2 border-background">
					<AvatarImage src={`https://github.com/shadcn${i}.png`} alt={`User ${i}`} />
...
export const Interactive: Story = {
	render: () => (
		<div className="flex gap-4">
			{[1, 2, 3].map((i) => (
				<Avatar key={i} className="cursor-pointer transition-transform hover:scale-110">
					<AvatarImage src={`https://github.com/shadcn${i}.png`} alt={`User ${i}`} />
					<AvatarFallback>U{i}</AvatarFallback>
				</Avatar>
			))}
