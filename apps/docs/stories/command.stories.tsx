...
export const WithSections: Story = {
	render: function SectionsCommand() {
		...
		const filteredSections = Object.entries(sections)
			.map(([heading, items]) => ({
				heading,
				items: items.filter((item) => item.label.toLowerCase().includes(inputValue.toLowerCase())),
			}))
			.filter((section) => section.items.length > 0);

		return (
			<Command className="rounded-lg border shadow-md">
				<CommandInput placeholder="Type to search..." value={inputValue} onValueChange={setInputValue} />
				<CommandList>
					<CommandEmpty>No results found.</CommandEmpty>
