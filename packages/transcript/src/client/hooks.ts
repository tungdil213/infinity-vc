...
import type { ITranscriptMessage, ConnectionState, SubscriptionOptions, MessageHandler } from '../core/types.js';
 ...
 */
export function createTranscriptHooks(React: {
	...
	function useChannels<TPayload = unknown>(
		...
		useEffect(() => {
			...
			const subscriptions = channels.map((channel) => {
				...
				return client.subscribe<TPayload>(
					channel,
					(msg) => {
