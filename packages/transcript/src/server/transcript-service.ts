...
export abstract class BaseTranscriptService implements ITranscriptService {
	...
	async sendToConnection<TPayload>(connectionId: string, type: string, payload: TPayload): Promise<void> {
	...
	protected createMessage<TPayload>(channel: string, type: string, payload: TPayload): ITranscriptMessage<TPayload> {
		return {
