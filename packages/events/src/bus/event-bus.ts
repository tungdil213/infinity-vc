...
import type { IEvent } from '../core/event.js';
import type { IEventHandler } from '../core/handler.js';
import type { SubscriberCallback, SubscriptionOptions } from '../core/subscription.js';
			...
			if (subscribers) {
				...
				if (index !== -1) {
					...
					if (this.stats.subscribersByType[eventType] === 0) {
						this.stats.subscribersByType = Object.fromEntries(
							Object.entries(this.stats.subscribersByType).filter(([type]) => type !== eventType)
						);
					}
				}
			}
