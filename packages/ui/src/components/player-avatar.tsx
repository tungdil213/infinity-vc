	...
	const avatarContent = (
		<div className={cn('relative inline-block', className)}>
			<Avatar className={config.avatar}>
				...
				<AvatarFallback className={config.text}>{player.nickName.charAt(0).toUpperCase()}</AvatarFallback>
			</Avatar>
				...
				<TooltipProvider>
					<Tooltip>
						<TooltipTrigger asChild>
							...
							>
								<span className={cn('text-gray-600 font-medium', config.text)}>+{remainingCount}</span>
							</div>
						</TooltipTrigger>
						<TooltipContent>
							<div>
