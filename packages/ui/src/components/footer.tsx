			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="grid grid-cols-1 md:grid-cols-4 gap-8">
					<div className="col-span-1 md:col-span-2">
						...
						<p className="text-main-foreground/70 mb-4">{description}</p>
					</div>
						...
						<div key={index}>
							...
							<ul className="space-y-2 text-main-foreground/70">
								{section.links.map((link, linkIndex) => (
									<li key={linkIndex}>
										{link.onClick ? (
											<button onClick={link.onClick} className="hover:text-main-foreground transition-colors">
												{link.label}
											</button>
										) : (
											<a href={link.href} className="hover:text-main-foreground transition-colors">
												{link.label}
											</a>
										)}
									</li>
